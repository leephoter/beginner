...
function checkresult(lineth, spaceth) { //빙고가 됐는지 체크하는 함수
    var whowin = false;
    ...
    && spaces[2][2].textContent === turn) { // 대각선 검사
        whowin = true;
    }
    if (spaces[0][2].textContent === turn && spaces[1][1].textContent === turn
    && spaces[2][0].textContent === turn) { // 대각선 검사
        whowin = true;
    }
    return whowin;
}
function reset(nowin) {  //승자가 나오면 종료, 리셋, 결과를 알려주는 함수
    if (nowin) {
        result.textContent = 'No whowin ! Rematch !'
    } else {
