...
screen.addEventListener('click', function () {
  ...
  if (screen.classList.contains('waiting')) {
    ...
    //
    if (!starttime) {
      // 부정 클릭 (시작시간이 없는데 클릭하면 (초록창이 뜨기전에 미리 클릭하면))
    ...
    if ((endtime - starttime) / 1000 <= 0.35) {
      alert("Fast reaction!! Let's check record on console.");
    } else if (
      (endtime - starttime) / 1000 > 0.35 &&
      (endtime - starttime) / 1000 <= 0.6
    ) {
      alert("It's normal rate!! Let's check record on console.");
    } else if ((endtime - starttime) / 1000 > 0.6) {
      alert("Try harder!! Let's check record on console.");
    }
