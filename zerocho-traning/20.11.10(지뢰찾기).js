...
document.querySelector('#exec').addEventListener('click', function () {
  ...
  //지뢰 위치 뽑기
  var subs = Array(hor * ver)
    .fill()
    .map(function (urea, index) {
      return index;
    });
  var shuffle = []; // 빈 배열
  ...
  for (var i = 0; i < ver; i += 1) {
    ...
    for (var j = 0; j < hor; j += 1) {
      ...
      td.addEventListener('contextmenu', function (e) {
        ...
        var space = Array.prototype.indexOf.call(
          prtr.children,
          e.currentTarget,
        );
      ...
      td.addEventListener('click', function (e) {
        ...
        if (
          [
            ...
            codetable.flagg,
            codetable.flagminee,
            codetable.aksemominee,
            codetable.askemo,
          ].includes(dataset[lines][space])
        ) {
          //열었던칸은 1의 data를 갖고있으니까 1인 곳을 클릭하면 클릭 이벤트 리턴
          return;
        }
        ...
        if (dataset[lines][space] === codetable.minee) {
          ...
          //지뢰가 아닌경우 주변 지뢰 갯수 세기
          dataset[lines][space] = 1;
          var periph = [dataset[lines][space - 1], dataset[lines][space + 1]];
          if (dataset[lines - 1]) {
            periph = periph.concat(
              dataset[lines - 1][space - 1],
              dataset[lines - 1][space],
              dataset[lines - 1][space + 1],
            );
          }
          if (dataset[lines + 1]) {
          ...
          if (purminenum === codetable.comtable) {
            //주변 지뢰갯수가 0이면 또 클릭
            var perspace = [];
            ...
            perspace
              .filter((v) => !!v)
              .forEach(function (sidespace) {
                var prtr = sidespace.parentNode;
                var prtbody = sidespace.parentNode.parentNode;
                var perspace2 = Array.prototype.indexOf.call(
                  prtr.children,
                  sidespace,
                );
                var perlines2 = Array.prototype.indexOf.call(
                  prtbody.children,
                  prtr,
                );
        ...
        if (opennum === hor * ver - mine) {
          //가로*세로 - 지뢰갯수 = 열 수 있는 모든 칸 <지뢰찾기 이겻을 때>
          stopflag = true;
